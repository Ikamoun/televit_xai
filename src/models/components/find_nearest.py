...
from .constant import SEASFIRE_MEAN, SEASFIRE_STD
from .dataset import resize_label


to_normalized_tensor = transforms.Compose([
    transforms.ToTensor(),
    transforms.Normalize(SEASFIRE_MEAN, SEASFIRE_STD)
])

# to_normalized_tensor = transforms.Compose([
#     transforms.Lambda(lambda x: x.unsqueeze(0)),  # Add batch dimension (1, 8, 128, 128)
#     transforms.Normalize(torch.tensor(SEASFIRE_MEAN), torch.tensor(SEASFIRE_STD))
# ])
